...
*/
	...
	char * recv_buffer = (char *)malloc(BUF_SIZE / KV_ITEM_SIZE * KV_ITEM_SIZE);
	len = mtcp_recv(ctx->mctx, sockid, recv_buffer, BUF_SIZE / KV_ITEM_SIZE * KV_ITEM_SIZE, 0);
...
*/
	...
	int res;
    while(recv_num > 0){
        struct kv_trans_item * recv_item = (struct kv_trans_item *)recv_buffer;
		sprintf(buff, "[SERVER] recv item len: %d\n", recv_item->len);
		fwrite(buff, strlen(buff), 1, fp);
		fflush(fp);
        if(recv_item->len > 0){
